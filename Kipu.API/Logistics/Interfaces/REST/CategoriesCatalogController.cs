...
public class CategoriesCatalogController(
    ...
    ILogger<CategoriesCatalogController> logger) : ControllerBase
{
    /// <summary>
    ...
    [HttpGet]
    [SwaggerOperation(
        Summary = "Gets all Categories Catalog",
        Description = "Gets all categories catalog",
        OperationId = "GetAllCategoriesCatalog")]
    [SwaggerResponse(200, "The categories catalog were found", typeof(IEnumerable<MaterialCategoryResource>))]
    public async Task<ActionResult> GetAllCategoriesCatalog(CancellationToken cancellationToken = default)
    {
        var query = new GetAllMaterialCategoriesQuery();
        var result = await materialCategoryQueryService.Handle(query, cancellationToken);
        var resources = result.Select(MaterialCategoryResourceFromEntityAssembler.ToResourceFromEntity);
        return Ok(resources);
    }
    ...
    /// </summary>
    /// <param name="id">The material category identifier</param>
    /// <param name="resource">The updated material category data</param>
    /// <param name="cancellationToken">Cancellation token</param>
    /// <returns>The updated material category</returns>
    [HttpPut("{id}")]
    [SwaggerOperation(
        Summary = "Updates a Material Category",
        Description = "Updates an existing material category with the given ID",
        OperationId = "UpdateMaterialCategory")]
    [SwaggerResponse(200, "The material category was updated", typeof(MaterialCategoryResource))]
    [SwaggerResponse(400, "The request payload is invalid", typeof(string))]
    [SwaggerResponse(404, "The material category was not found", typeof(string))]
    [SwaggerResponse(500, "Unexpected server error", typeof(ProblemDetails))]
    public async Task<ActionResult> UpdateMaterialCategory(int id,
        [FromBody] UpdateMaterialCategoryResource resource,
        CancellationToken cancellationToken)
    {
        try
        {
            var command = UpdateMaterialCategoryCommandFromResourceAssembler.ToCommandFromResource(id, resource);
            var result = await materialCategoryCommandService.Handle(command, cancellationToken);
            return ActionResultFromUpdateMaterialCategoryAssembler.ToActionResultFromUpdateMaterialCategoryResult(
                result, this, localizer);
        }
        catch (ArgumentException ex)
        {
            logger.LogWarning(ex, "Validation failed while updating material category with id {Id}", id);
            return BadRequest(localizer["InvalidMaterialCategoryRequest"].Value);
        }
        catch (Exception ex)
        {
            logger.LogError(ex, "Unexpected error while updating material category with id {Id}", id);
            return Problem(
                title: localizer["UnexpectedServerError"].Value,
    ...
    [HttpDelete("{id}")]
    [SwaggerOperation(
        Summary = "Deletes a Material Category",
        Description = "Deletes an existing material category with the given ID",
        OperationId = "DeleteMaterialCategory")]
    [SwaggerResponse(204, "The material category was deleted")]
    [SwaggerResponse(404, "The material category was not found", typeof(string))]
    [SwaggerResponse(500, "Unexpected server error", typeof(ProblemDetails))]
    public async Task<ActionResult> DeleteMaterialCategory(int id,
        CancellationToken cancellationToken)
    {
        try
        {
            var result = await materialCategoryCommandService.HandleDelete(id, cancellationToken);
            if (result is Result<MaterialCategory, UpdateMaterialCategoryError>.Success)
                return NoContent();
            var failure = (Result<MaterialCategory, UpdateMaterialCategoryError>.Failure)result;
            return failure.Error switch
            {
                UpdateMaterialCategoryError.MaterialCategoryNotFound =>
                    NotFound(localizer["MaterialCategoryNotFound"].Value),
                _ => Problem(
                    title: localizer["UnexpectedServerError"].Value,
                    detail: localizer["UnexpectedErrorDeletingMaterialCategory"].Value,
                    statusCode: 500)
            };
        }
        catch (Exception ex)
        {
            logger.LogError(ex, "Unexpected error while deleting material category with id {Id}", id);
            return Problem(
                title: localizer["UnexpectedServerError"].Value,
                detail: localizer["UnexpectedErrorDeletingMaterialCategory"].Value,
                statusCode: 500);
